...
function Main() {
    [[ "${GENESIS_NODE}" != "$(hostname)" ]] && $InfoMessage "Not the genesis node" && exit 0
    ...
    if [[ ${nUserKeys} -ne 2 ]] || [[ ${nValidatorKeys} -ne 2 ]]; then
        $ErrMessage "User or Validator keys are missing" && exit 1
    fi
    ...
    [[ $? -ne 0 ]] && $ErrMessage "Failed creating settings proposal" && exit 1
    ...
    [[ $? -ne 0 ]] && $ErrMessage "Failed initializing the consensus settings" && exit 1
    ...
    [[ $? -ne 0 ]] && $ErrMessage "Failed to create a batch to register the first Sawtooth node" && exit 1
